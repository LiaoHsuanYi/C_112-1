...
int main()
{
    ...
    int output[4] = {0}; //c & p1~p3's output
    int bets[3] = {0}; 
    ...
    while (count <= players)
    {
        ...
        while (str[0] == 'Y')
        {
            ...
            if (points[count] > 10.5)
            {
                burst[count - 1] = true;
                stop[count - 1] = true;
                output[count] -= bets[count - 1]; //player lose money
                output[0] += bets[count - 1]; //computer earn player's bets
                break;
            }
            ...
            {
                stop[count - 1] = true;
                output[count] += bets[count - 1]; //player earn money
                output[0] -= bets[count - 1]; //computer lose player's bets
                break;
            }
            ...
            {
                stop[count - 1] = true;
                output[count] += bets[count - 1];
                output[0] -= bets[count - 1];
                break;
            }
    ...
    if (end == false)
    {
        //if computer's points < all players's minimum points
        while (points[0] < min_point)
        {
            ...
            if (points[0] > 10.5)
            {
                for (int k = 0; k < players; k++)
                {
                    if (stop[k] == false)
                    {
                        output[k + 1] += bets[k]; //all players earn
                        output[0] -= bets[k]; //computer lose
                    }
        ...
        if (points[0] <= 10.5)
        {
            for (int k = 0; k < players; k++)
            {
                if (stop[k] == false)
                {
                    ...
                    {
                        output[k + 1] -= bets[k]; //players lose
                        output[0] += bets[k]; //computer earn
                    }
                    else
                    {
                        output[k + 1] += bets[k];//players earn
                        output[0] -= bets[k];//computer lose
                    }
